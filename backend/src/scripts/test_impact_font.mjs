...
function createBadgeSvg(opt) {
  ...
  // Available text width inside each card box: (588 - 186) = 402px, with padding ~360-380px
  const TEXT_AVAIL_W = L_RIGHT - L_DIV - 24; // ~378px
  ...
  ${opt.textMarkup(L_TEXT_CENTER, R_TEXT_CENTER, BADGE_CY, TEXT_AVAIL_W)}
...
async function run() {
  ...
  // Impact Condensed style matching exact screenshot
  const textMarkupOpt = (L_CENTER, R_CENTER, CY, availW) => `
    <!-- Left: 10 YEARS & LONG LIFE -->
    <text x="${L_CENTER}" y="${CY - 2}" 
          font-family="Impact, 'Arial Black', sans-serif" 
          font-size="68" 
          fill="#111111" 
          text-anchor="middle"
          textLength="360"
          lengthAdjust="spacingAndGlyphs">10 YEARS</text>

    <text x="${L_CENTER}" y="${CY + 40}" 
          font-family="Impact, 'Arial Black', sans-serif" 
          font-size="36" 
          fill="#111111" 
          text-anchor="middle"
          letter-spacing="4"
          textLength="310"
          lengthAdjust="spacing">LONG LIFE</text>

    <!-- Right: CARD WARRANTY & 3 YEARS -->
    <text x="${R_CENTER}" y="${CY - 20}" 
          font-family="Impact, 'Arial Black', sans-serif" 
          font-size="34" 
          fill="#111111" 
          text-anchor="middle"
          letter-spacing="2"
          textLength="350"
          lengthAdjust="spacing">CARD WARRANTY</text>

    <text x="${R_CENTER}" y="${CY + 42}" 
          font-family="Impact, 'Arial Black', sans-serif" 
          font-size="70" 
          fill="#111111" 
          text-anchor="middle"
          textLength="340"
          lengthAdjust="spacingAndGlyphs">3 YEARS</text>
  `;

  const svgStr = createBadgeSvg({ textMarkup: textMarkupOpt });
  ...
  const previewCrop = await sharp(resultBuf)
    ...
    .png()
    .toFile(path.resolve(__dirname, '../assets/impact_font_preview.png'));

  console.log('✓ Generated impact font preview');
}
