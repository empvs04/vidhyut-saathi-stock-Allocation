...
export function buildTallBoxBadgeSvg() {
  const boxW = 556;
  const boxH = 128;
  const leftMargin = 32;
  ...
  <!-- Left Box: Tall Bold 10 YEARS (Natural un-stretched) & LONG LIFE -->
  <g transform="translate(${L_TEXT_CENTER}, ${BADGE_CY - 3}) scale(0.96, 1.18)">
    <text x="0" y="0" class="font-heavy" font-size="64" letter-spacing="-0.5">10 YEARS</text>
  </g>
  <text x="${L_TEXT_CENTER}" y="${BADGE_CY + 41}" class="font-heavy" font-size="33" letter-spacing="3.5">LONG LIFE</text>

  <!-- Right Box: CARD WARRANTY & Tall Bold 3 YEARS (Natural un-stretched) -->
  <text x="${R_TEXT_CENTER}" y="${BADGE_CY - 20}" class="font-bold" font-size="30" letter-spacing="1.5">CARD WARRANTY</text>
  <g transform="translate(${R_TEXT_CENTER}, ${BADGE_CY + 42}) scale(0.96, 1.18)">
...
export async function applyTallBoxDesign() {
  ...
  console.log('\n✅ Tall un-stretched badge typography applied to all production assets!');
}

applyTallBoxDesign().catch(console.error);
