...
export function buildFullBoxBadgeSvg() {
  const boxW = 556;
  const boxH = 128;
  const leftMargin = 32;
  ...
  <!-- Left Box: 10 YEARS & LONG LIFE spanning full box width -->
  <text x="${L_TEXT_CENTER}" y="401" class="font-heavy" font-size="64" letter-spacing="-0.5" textLength="364" lengthAdjust="spacingAndGlyphs">10 YEARS</text>
  <text x="${L_TEXT_CENTER}" y="445" class="font-heavy" font-size="34" letter-spacing="4" textLength="320" lengthAdjust="spacing">LONG LIFE</text>

  <!-- Right Box: CARD WARRANTY & 3 YEARS spanning full box width -->
  <text x="${R_TEXT_CENTER}" y="377" class="font-bold" font-size="31" letter-spacing="2.5" textLength="340" lengthAdjust="spacing">CARD WARRANTY</text>
  <text x="${R_TEXT_CENTER}" y="445" class="font-heavy" font-size="66" letter-spacing="-0.5" textLength="340" lengthAdjust="spacingAndGlyphs">3 YEARS</text>

</svg>`;
}

export async function applyFullBoxDesign() {
  const baseTemplate = path.resolve(__dirname, '../../../VidhyutSaathi_Label_2x1.5in.png');
  const baseBuf = fs.readFileSync(baseTemplate);

  const svgStr = buildFullBoxBadgeSvg();
  ...
  console.log('\n✅ Full box aligned badge typography applied to all production assets!');
}

applyFullBoxDesign().catch(console.error);
