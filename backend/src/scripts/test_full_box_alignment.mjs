...
function createSvgVariant(textSvg) {
  const boxW = 556;
  const boxH = 128;
  const leftMargin = 32;
...
async function testAll() {
  ...
  // Option 1: Full box span with Arial Black & bold uppercase
  const opt1 = `
    <!-- Left Box -->
    <text x="${L_CENTER}" y="401" class="font-heavy" font-size="64" letter-spacing="-0.5" textLength="364" lengthAdjust="spacingAndGlyphs">10 YEARS</text>
    <text x="${L_CENTER}" y="445" class="font-heavy" font-size="34" letter-spacing="4" textLength="320" lengthAdjust="spacing">LONG LIFE</text>

    <!-- Right Box -->
    <text x="${R_CENTER}" y="377" class="font-bold" font-size="31" letter-spacing="2.5" textLength="340" lengthAdjust="spacing">CARD WARRANTY</text>
    <text x="${R_CENTER}" y="445" class="font-heavy" font-size="66" letter-spacing="-0.5" textLength="340" lengthAdjust="spacingAndGlyphs">3 YEARS</text>
  `;

  const svg1 = createSvgVariant(opt1);
  const buf1 = await sharp(baseBuf).composite([{ input: Buffer.from(svg1), top: 0, left: 0 }]).png().toBuffer();
  await sharp(buf1).extract({ left: 0, top: 310, width: 1200, height: 190 }).png().toFile(path.resolve(__dirname, '../assets/full_box_preview.png'));

  console.log('✓ Created full_box_preview.png');
}
