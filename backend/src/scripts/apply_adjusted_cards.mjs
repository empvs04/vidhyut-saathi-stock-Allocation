...
export function buildAdjustedBadgeSvg() {
  const boxW = 556;
  const boxH = 128;
  const leftMargin = 32;
...
export async function applyAdjustedDesign() {
  ...
  console.log('\n✅ Adjusted design applied to all production label templates!');
}

applyAdjustedDesign().catch(console.error);
