...
function createBadgeSvg({ boxH, divH }) {
  const boxW = 556;
  const leftMargin = 32;
...
async function run() {
  ...
  // Height increased to 138px (taller cards), Divider height to 102px
  const svgStr = createBadgeSvg({ boxH: 138, divH: 102 });
  ...
  await sharp(resultBuf)
    ...
    .png()
    .toFile(path.resolve(__dirname, '../assets/expanded_text_preview.png'));

  console.log('✓ Generated expanded text preview');
}
